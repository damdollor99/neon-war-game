...
'use strict';
{
    ...
    window.RuntimeInterface = class RuntimeInterface {
        ...
        _GetWorkerURL(url) {
            ...
            else if (url.endsWith("/workermain.js") &&
                this._workerScriptURLs.hasOwnProperty("workermain.js")) ret = this._workerScriptURLs["workermain.js"];
...
'use strict';
{
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            this._enableAndroidVKDetection =
                false;
            ...
            iRuntime.AddRuntimeComponentMessageHandler("runtime",
                "script-create-worker", e => this._OnScriptCreateWorker(e));
            ...
            iRuntime.AddRuntimeComponentMessageHandler("runtime",
                "exported-to-video", e => this._OnExportedToVideo(e));
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen["orientation"] && screen["orientation"]["lock"]) screen["orientation"]["lock"](orientation).catch(err =>
        ...
        _OnMouseEventAsPointer(name, e) {
            ...
            if (name === "pointerdown" &&
                lastButtons !== 0) name = "pointermove";
        ...
        _HandlePointerDownFocus(e) {
            ...
            if (this._IsElementCanvasOrDocument(e.target) && document.activeElement &&
                !this._IsElementCanvasOrDocument(document.activeElement)) document.activeElement.blur()
        }
        ...
        _OnExportVideoProgress(e) {
            ...
            if (this._exportVideoUpdateTimerId === -1) this._exportVideoUpdateTimerId = setTimeout(() => this._DoUpdateExportVideoProgressMessage(), 250)
        }
    ...
    self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
        ...
        async _CreateAudioContext(e) {
            ...
            if (this._iRuntime.IsExportingToVideo()) {
                ...
                if (!this.SupportsWebMOpus()) opts["sampleRate"] =
                    48E3;
            ...
            try {
                await Promise.all(e["preloadList"].map(o => this._GetAudioBuffer(o["originalUrl"],
                    o["url"], o["type"], false)))
            } catch (err) {
        ...
        _AttachUnblockEvents() {
            ...
            window.addEventListener("click", this._unblockFunc,
                true);
        ...
        async _Play(e) {
            ...
            if (startTime > 0 && !e["trueClock"])
                if (this._audioContext["getOutputTimestamp"]) {
                    ...
                    startTime = startTime - outputTimestamp["performanceTime"] /
        ...
        _OnTick(e) {
            ...
            for (const instPan of e["instPans"]) {
                ...
                for (const ai of this._audioInstances)
                    if (ai.GetUID() === uid) ai.SetPanXYA(instPan["x"],
                        instPan["y"], instPan["angle"])
            }
        }
        async _AddEffect(e) {
            const type = e["type"];
            ...
            if (type === "filter") effect = new self.C3AudioFilterFX(this, ...params);
                ...
                effect =
                    new self.C3AudioConvolveFX(this, buffer.GetAudioBuffer(), ...params);
        ...
        async _OnLoadState(e) {
            ...
            if (saveLoadMode !== 3) {
                ...
                for (const ai of this._audioInstances)
                    if (ai.IsMusic() && saveLoadMode === 1 || !ai.IsMusic() && saveLoadMode ===
                        2) keepAudioInstances.push(ai);
                    else ai.Release();
...
'use strict';
{
    ...
    self.C3AudioInstance = class C3AudioInstance {
        ...
        Tick(currentTime) {
            if (this._fadeEndTime !==
                -1 && currentTime >= this._fadeEndTime) {
                this._fadeEndTime = -1;
        ...
        SetPan(x, y, angle, innerAngle,
            outerAngle, outerGain) {
